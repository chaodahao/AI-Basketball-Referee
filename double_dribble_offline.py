...
class DoubleDribbleDetector:
    def __init__(self):
        ...
        self.ball_model = YOLOV5(BALL_MODEL_PATH)
        ...
        # Get other infomation for visulizing the resulting video
        self.frame_height = int(self.cap.get(4))
        self.fps = self.cap.get(cv2.CAP_PROP_FPS) #帧率

        # visulizing the resulting video
        self.out = cv2.VideoWriter(
            os.path.join(SAVE_BASE_PATH,"double_dribble_{}_{}.mp4".format(time.strftime("%Y%m%d-%H%M%S"), video_name.split(".")[0])), 
